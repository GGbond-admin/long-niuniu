...
import {
  profitPoolUserOptionListWhere,
  profitPoolUserOptionWhere,
} from './profitPool.js';
...
describe('利润池用户选择器过滤', () => {
  ...
  it('建第一层空搜索只拉未归属的启用用户', () => {
    const where = profitPoolUserOptionListWhere('', 'agent');
  ...
  it('搜索 UID 时仍包含已归属玩家，方便解绑后建成第一层', () => {
    const where = profitPoolUserOptionListWhere('10002', 'agent');
    expect(where).not.toHaveProperty('agentBinding');
    expect(where).not.toHaveProperty('agentProfile');
