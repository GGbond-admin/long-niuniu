...
- 点数：三位关键数字求和取个位；和为 10 → 牛牛；个位 0 且非牛牛 → 0 点
- 牌型：豹子 17x / 满牛 15x / 反顺 14x / 顺子 13x / 对子 12x / 金牛 11x / 牛牛 10x / 普通（点位倍数可配）
- 比牌：豹子 > 满牛 > 顺子 > 反顺 > 对子 > 金牛 > 牛牛 > 普通；同级按牌型规则（普通比点数；对子先后两位再前位；金牛比中间位；其余比金额）
- 自爆：可后台开关；开启时普通牌型 ≤3 点判输；双自爆庄赢；特殊牌型豁免
- 动态范围：普通满注=庄钱×后台比例、满梭哈=庄钱×后台比例；最低额默认下注 RM3 / 梭哈 RM30
